...
function updateTickerUI(data) {
    ...
    // Create a span for each piece of data
    ...
    // Set the class on the ticker
    ticker.className = data.change.startsWith('-') ? 'price-down' : 'price-up';

    // Append all spans to the ticker div
    ticker.appendChild(nameSpan);
    ticker.appendChild(priceSpan);
    ticker.appendChild(percentSpan);
...
// Set an interval to reload data every 2 minutes
